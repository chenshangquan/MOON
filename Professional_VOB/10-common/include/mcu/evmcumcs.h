...
//3.6.1.1.1	请求录像
    ...
    // + TEqp(所要使用得录像机) + TRecStartPara(录像参数) + 录像名(0结尾字符串)
...
//3.6.1.2	放像操作
//3.6.1.2.1	开始播放
	//会控开始放像请求, 消息体：  TEqp + u8(是否双流放像) + 录像名(0结尾字符串)
...
//mcu配置界面化
    ...
    //+u8(prs个数)+TPrsCfgInfo+u8(复合电视墙个数)+TMTvwallInfo数组
	//+T8000BDscMod+u8(HDbas个数)+TEqpBasHDCfgInfo数组
	...
	//mcs修改mcu配置信息基本设置 消息体：TMcuGeneralCfg
...
//N+1备份消息
	...
	//VMP优先级抢占请求，消息体：TMt + TChnnlMemberInfo[] + u8
