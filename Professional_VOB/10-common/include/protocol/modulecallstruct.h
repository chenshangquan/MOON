...
#define MAX_CONF_AUDMIX_NUM_TP	4			//最大混音参与数量
...
typedef struct tagTMdlConfAuxMixInfoTP
{
	BOOL bStart;
	u16	wConfID;
	u16 wSpeakerIndex;					//下面数组的索引
	u16	awList[MAX_CONF_AUDMIX_NUM_TP];	//混音ID
...
typedef struct tagTerminalAddress
{
	...
	s8	achAlias[MAX_LEN_CALL_ALIAS + 1];	//终端别名或E164号
...
struct TVideoFormatCapbility
{
	...
	EmModuleProfileMask m_emProfile; //h264用
...
enum EmChannelReason
{
	...
	emModuleDualNotSend,	//在adapter层发现协议栈状态不对，双流未经协议栈发送出去（当前状态不正确，导致协议栈未发送）
	emModuleDualNotSendMdl,	//在module层发现能力状态不对直接回调结果给业务（仅协议栈内部使用，业务回调无此原因）
};
