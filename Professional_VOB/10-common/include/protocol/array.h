...
public :
	...
	virtual int StoreElement( T &Element, u32 dwIndex ) = 0;
	...
	// 在有序数组中查找指定元素
	int SearchElemByBinary( T& KeyElement );
...
template< class T >
int CAbArray< T >::SearchElemByBinary( T& KeyElement )
{
	int nLowPos = 0;
...
public :
	...
	int StoreElement( T &Element, u32 dwIndex );
...
template< class T >
int CAddArray< T >::StoreElement( T &Element, u32 dwIndex )
{
	if ( m_pArr == NULL )
	{
