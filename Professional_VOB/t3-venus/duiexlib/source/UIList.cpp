...
void CListUI::DoEvent(TEventUI& event)
{
	...
	{
		//实现列表拖拽
		if ( g_bLButtonDown && !m_bDragWndIng )
		{
			int nYPos = 0;
			int cyLine = 0;
			SIZE sz;
	...
	{
		if( IsEnabled() ) {
			m_uButtonState |= UISTATE_HOT;    //remark 去除行高亮效果
...
{
	m_pList->SetBkImage(this->GetBkImage());
	m_pList->SetMosaicBk(this->GetMosaicBk());
...
bool CListElementUI::Activate()
{
    ...
    if( m_pManager != NULL ) m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMDBCLICK);       //itemactivate 双击列表条目
    return true;
}
...
void CListElementUI::DoEvent(TEventUI& event)
{
	...
	if ( event.Type == UIEVENT_GESTURENOTIFY )  //同if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK )
	{
		return;
	}
...
void CListLabelElementUI::DoEvent(TEventUI& event)
{
    if( !IsMouseEnabled() && event.Type > UIEVENT__MOUSEBEGIN && event.Type < UIEVENT__MOUSEEND ) {
        ...
        //return;   //remark 此函数所有return防止截获到LBUTTONDOWN后  屏蔽了WM_GESTURE等消息  
    }
...
//         }
	...
	if ( event.Type == UIEVENT_GESTURE ) 
	{
		CControlUI::DoEvent(event);     //派发WM_GESTURENOTIFY等消息到CListUI   实现滚动
...
bool CListContainerElementUI::Activate()
{
    ...
    if( m_pManager != NULL ) m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMDBCLICK);
    return true;
}
...
void CListContainerElementUI::DoEvent(TEventUI& event)
{
    ...
    {
        if( IsEnabled() ){
            m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMCLICK);        //itemactivate 单击列表条目
            Select();
            Invalidate();
        }
...
void CListContainerElementUI::PreDrawMosaicSelectItemBk()
{
	m_strItemSelectLeft = m_strMosaicItemImg.c_str();
	m_strItemSelectLeft += _T("\\");
	m_strItemSelectLeft += pItemSelectLeft;

	m_strItemSelectMiddle = m_strMosaicItemImg.c_str();
	m_strItemSelectMiddle += _T("\\");
	m_strItemSelectMiddle += pItemSelectMiddle;

	m_strItemSelectRight = m_strMosaicItemImg.c_str();
	m_strItemSelectRight += _T("\\");
	m_strItemSelectRight += pItemSelectRight;
}

void CListContainerElementUI::DrawMosaicSelectItemBk(HDC hDC, const RECT& rcItem)
{
	int nFixedWidth = 0;
	if ( this->m_pParent->GetScrollVisible() )
	{
		nFixedWidth = rcItem.right - rcItem.left - m_nSelBkPos;		
	}
	else
	{
		nFixedWidth = rcItem.right - rcItem.left - m_nSelBkPos * 2;
	}

	if( !m_strItemSelectLeft.empty() && !m_strItemSelectMiddle.empty() && !m_strItemSelectRight.empty() ) 
	{
		CControlUI::MosaicHoriImage(hDC, m_strItemSelectLeft.c_str(), m_strItemSelectRight.c_str(), m_strItemSelectMiddle.c_str(),
			nFixedWidth, TRUE, CPoint(m_nSelBkPos, 0) );
	}
}

void CListContainerElementUI::DrawMosaicItemBk(HDC hDC, const RECT& rcItem)
{
	ASSERT(m_pOwner);
	if( m_pOwner == NULL ) return;
	...
	if( IsSelected() ) {
		//iBackColor = pInfo->dwSelectedBkColor;
		DrawMosaicSelectItemBk(hDC, rcItem);
	}
	if( !IsEnabled() ) {
		iBackColor = pInfo->dwDisabledBkColor;
	}
...
void CListContainerElementUI::SetMosaicItemImage(LPCTSTR pStrImage)
{
	m_strMosaicItemImg = pStrImage;
	//Invalidate();
}

LPCTSTR CListContainerElementUI::GetMosaicItemImage()
{
	return m_strMosaicItemImg.c_str();
}
