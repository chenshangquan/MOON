...
TCHAR* CPaintManagerUI::m_pStrResourcePath = NULL;
TCHAR* CPaintManagerUI::m_pStrResourceZip = NULL;
...
//CStdPtrArray CPaintManagerUI::m_aCustomFonts; //2013.6.14 by yjj
CStdStringPtrMap CPaintManagerUI::m_mImageHash; //2013.6.14 by yjj
...
CPaintManagerUI::~CPaintManagerUI()
{
	...
	if ( NULL != m_pCurrentGestureInfo )
	{
		SAFE_RELEASE(m_pCurrentGestureInfo);
	}
}

bool CPaintManagerUI::LoadGestureLib()
{
	static HMODULE hUserDll = ::LoadLibrary(_T("user32.dll"));
	if( hUserDll == NULL )
	{
		return false;
	}

	m_pfGetGestureInfo = (GETGESTUREINFO)GetProcAddress(hUserDll, "GetGestureInfo");
...
TCHAR* CPaintManagerUI::GetInstancePath()
{
	...
	memset( tszModule, 0, MAX_PATH + 1 );
	wcscat( tszModule, sInstancePath.c_str() );
	return tszModule;
}
...
const TCHAR* CPaintManagerUI::GetResourcePath()
{
	...
	if ( NULL != m_pStrResourcePath )
	{
		wcscat( chPath, m_pStrResourcePath );
	}
...
{
	TCHAR chPath[MAX_PATH] = {0};
	if ( NULL != m_pStrResourceZip )
	{
		wcscat( chPath, m_pStrResourceZip );
	}
...
void CPaintManagerUI::SetResourcePath(const TCHAR* pStrPath)
{
	if ( NULL == m_pStrResourcePath )
	{
		m_pStrResourcePath = const_cast<TCHAR*>(pStrPath);
		if( wcslen(m_pStrResourcePath) == 0 ) return;
		TCHAR cEnd = m_pStrResourcePath[ wcslen(m_pStrResourcePath) - 1];
		if( cEnd != _T('\\') && cEnd != _T('/') ) 
		{
			wcscat( m_pStrResourcePath, _T("\\") );
		}
	}
}

void CPaintManagerUI::SetResourceZip(const TCHAR* pStrPath)
{
    m_pStrResourceZip = const_cast<TCHAR*>(pStrPath);
}
...
//#endif
	...
	case WM_GESTURE:
		{	
			OnGestureHandler( uMsg, wParam, lParam, lRes );
		}
		break;
...
void CPaintManagerUI::OnGestureHandler( UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT& lRes )
{
	...
	if ( NULL == m_pfGetGestureInfo )
	{
		return;
	}

	if (!(*m_pfGetGestureInfo)((HGESTUREINFO)lParam, m_pCurrentGestureInfo) /*|| m_pCurrentGestureInfo->hwndTarget != GetSafeHwnd()*/)
	{
		ZeroMemory(m_pCurrentGestureInfo, sizeof(GESTUREINFO));
		return;
	}
	...
	POINT ptGetGestury;
	ptGetGestury.x = m_pCurrentGestureInfo->ptsLocation.x - rectWnd.left;
	ptGetGestury.y = m_pCurrentGestureInfo->ptsLocation.y - rectWnd.top;


	CControlUI* pControl = FindControl(ptGetGestury);    
	if( pControl == NULL ) return;
	if( pControl->GetManager() != this ) return;

	if( !pControl->IsGesture() )
	{
		return;
	}
	...
	//BOOL bResult = SetGestureConfig( m_pCurrentGestureInfo->hwndTarget, 0, uiGcs, &gc, sizeof(GESTURECONFIG) );  
	//if( bResult )
	//{
	//	bResult = 0;
	//}
	...
	//接收不带槽和延时的水平与垂直单指平移。这是二维导航应用程序（如 Microsoft Surface Globe 应用程序）的典型配置
	...
	TGestureInfo tGestureInfo;
	BOOL bGestureState = FALSE;
	switch (m_pCurrentGestureInfo->dwID)
	{
	case GID_BEGIN:
		{
			tGestureInfo.emGestureType = emGestureBeg;
			m_ptGestureFrom = ptGetGestury;
			m_ulGestureArg = m_pCurrentGestureInfo->ullArguments;
		}
		break;
	case GID_END:
		{
			tGestureInfo.emGestureType = emGestureEnd;
			m_ptGestureFrom = CPoint(-1, -1);
			m_ulGestureArg = 0;
			ZeroMemory(m_pCurrentGestureInfo, sizeof(GESTUREINFO));
		}
	...
	case GID_ZOOM:
		{
			...
			tGestureInfo.emGestureType = emGestureZoom;
			bGestureState = TRUE;

			long lDelta = (long)(m_pCurrentGestureInfo->ullArguments - m_ulGestureArg);
			...
			else if (abs((int)lDelta) > 5)
			{
				...
				m_ptOffset.x += (int)(.5 + dblDelta * (m_ptOffset.x - ptGetGestury.x));
				m_ptOffset.y += (int)(.5 + dblDelta * (m_ptOffset.y - ptGetGestury.y));
			}

			tGestureInfo.dblZoomRatio = m_dblZoomRatio;
		}
		break;
	case GID_PAN:
		{
			if ( m_bInZoom || m_bInRotate )
			{
				return;
			}
			...
			const int dx = ptGetGestury.x - m_ptGestureFrom.x;
			const int dy = ptGetGestury.y - m_ptGestureFrom.y;
			...
			else if ( dx != 0 || dy != 0 )
			{
				...
				if ( abs(m_ptOffset.x) >= abs(m_ptOffset.y) )   //水平PAN 否则为垂直PAN
				{
					tGestureInfo.emGestureType = emGestureHorPan;
					tGestureInfo.ptOffset.x = dx;
				}
				else
				{
					tGestureInfo.emGestureType = emGestureVerPan;
					tGestureInfo.ptOffset.y = dy;
				}
			}

			tGestureInfo.dblZoomRatio = m_dblZoomRatio;			//当收到Pan将之前缩放比传递赋值

			//设置手势函数参数
			tGestureInfo.ptFrom = m_ptGestureFrom;
			tGestureInfo.ptTo = ptGetGestury;
		}
		break;
	case GID_ROTATE:
		{
			if ( m_bInZoom || m_bInPan )
			{
				return;
			}

			tGestureInfo.emGestureType = emGestureRotate;
			bGestureState = TRUE;
			...
			{
				m_bInRotate = TRUE;
				tGestureInfo.bEndRotate = false;

				m_dblZoomRatio = 1.0;   //旋转将图片大小还原
				tGestureInfo.dblZoomRatio = m_dblZoomRatio;
			}
			else if ( (m_pCurrentGestureInfo->dwFlags & GF_END) == GF_END )
			{
				m_bInRotate = FALSE;
				tGestureInfo.bEndRotate = true;
			}
			else if ( dx != 0 || dy != 0 )
			{
				//m_ptOffset.x /*+*/= dx;
				...
				if ( dy > 0 )   
				{
					tGestureInfo.emGestureType = emGestureCWRotate;
					//tGestureInfo.ptOffset.x = dx;
				}
				else
				{
					tGestureInfo.emGestureType = emGestureACWRotate;
					//tGestureInfo.ptOffset.y = dy;
				}
			}
	...
	case GID_TWOFINGERTAP:
		{
			tGestureInfo.emGestureType = emGestureFingerTap;
			bGestureState = TRUE;
	...
	case GID_PRESSANDTAP:
		{
			...
			tGestureInfo.emGestureType = emGesturePressAndTap;
			bGestureState = TRUE;
		}
		break;
	default:
	...
	event.wParam = wParam;
	event.lParam = lParam;
	event.ptMouse = ptGetGestury;
	event.wKeyState = (WORD)wParam;
	event.dwTimestamp = ::GetTickCount();
	event.tGestureInfo = tGestureInfo;
	pControl->Event(event);
	m_pEventClick = pControl;
}
...
void CPaintManagerUI::ReapObjects(CControlUI* pControl)
{
    ...
    KillTimer(pControl);
    /*const CStdString&*/const tString& sName = pControl->GetName();
    if( !sName.empty() ) {
...
CControlUI* CALLBACK CPaintManagerUI::__FindControlFromNameHash(CControlUI* pThis, LPVOID pData)
{
    ...
    /*const CStdString&*/ const tString& sName = pThis->GetName();
...
bool CPaintManagerUI::DoCase( const LPCTSTR szCaseName )
{
	return m_cCaseManager.DoCase( szCaseName, this );
}
