...
TImageInfo* CRenderEngine::LoadImage(STRINGorID bitmap, LPCTSTR type, DWORD mask, bool bRelativePath )
{
    LPBYTE pData = NULL;
    DWORD dwSize = 0;
    ...
    if( type == NULL ) 
	{
		 //默认为相对路径  否则为绝对路径 ldy 2013.06.06
		TCHAR chFile[MAX_PATH] = {0};  
		tString strTmpFile(bitmap.m_lpstr);
        ...
        if( wcslen(CPaintManagerUI::GetResourceZip()) == 0 ) 
		{
			wcscat( chFile, bitmap.m_lpstr );
            HANDLE hFile = ::CreateFile( chFile, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, \
			...
			wcscat( chFile, CPaintManagerUI::GetResourceZip() );
            HZIP hz = OpenZip((void*)chFile, 0, 2);
            ZIPENTRY ze; 
            int i; 
...
bool CRenderEngine::DrawImageString(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, 
                                          LPCTSTR pStrImage, LPCTSTR pStrModify, bool bTiledX, bool bTiledY)
{
...
bool CRenderEngine::DrawImageGDIPlus( HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, \
	const TImageInfo* imgInfo, Image* pImg, TEventUI& event, CPoint ptInCtl, RECT rcItem, bool bSuportGesture )
{
	RECT rcTemp;
	if( !::IntersectRect(&rcTemp, &rcItem, &rc) ) return true;
	if( !::IntersectRect(&rcTemp, &rcItem, &rcPaint) ) return true;
	...
	if ( !bSuportGesture )
	{
		DrawGDIPlus( graphics, rc, rcPaint, imgInfo, pImg, ptInCtl, rcItem );
	}
	else
	{
		DrawGestureImage( graphics, hDC, rc, rcPaint, imgInfo, pImg, event, ptInCtl, rcItem );
	}

	return true;
}

bool CRenderEngine::DrawGDIPlus( Graphics& grps, const RECT& rc, const RECT& rcPaint, \
	const TImageInfo* imgInfo, Image* pImg, CPoint ptInCtl, RECT rcItem )
{
	if ( NULL != pImg && NULL != imgInfo )
	{
		int xPos = rcItem.left;
		int yPos = rcItem.top;

		int nImgWidth = imgInfo->nX;
		int nImgHeight = imgInfo->nY;

		grps.DrawImage( pImg, xPos + ptInCtl.x, yPos + ptInCtl.y, nImgWidth, nImgHeight );
	}

	return true;
}

bool CRenderEngine::DrawGestureImage( Graphics& grps, HDC hDC, const RECT& rc, const RECT& rcPaint, \
	const TImageInfo* imgInfo, Image* pImg, TEventUI& event, CPoint ptInCtl, RECT rcItem )
{
	if ( NULL != pImg && NULL != imgInfo )
	{
		...
		transformMatrix.Scale(Gdiplus::REAL(event.tGestureInfo.dblZoomRatio), Gdiplus::REAL(event.tGestureInfo.dblZoomRatio));

		grps.SetTransform(&transformMatrix);

		//if ( event.tGestureInfo.emGestureType == emGestureRotate ||    //去除判断   否则纵向显示  拖拽设置的滚动条标度不正确
		//	 event.tGestureInfo.emGestureType == emGestureACWRotate ||
		//	 event.tGestureInfo.emGestureType == emGestureACWRotate )
		{
			Gdiplus::SolidBrush br( GetBkColor(hDC) );
			grps.FillRectangle( &br, 0, 0, pImg->GetWidth()*event.tGestureInfo.dblZoomRatio, pImg->GetHeight()*event.tGestureInfo.dblZoomRatio );    //填充背景色为窗口色   防止多图层

			if ( event.tGestureInfo.dblAngle == 90 )
			{
				grps.ResetTransform();   //恢复默认绘制
				grps.TranslateTransform( pImg->GetHeight()*event.tGestureInfo.dblZoomRatio, 0 ); // 将原点移动到(pImg->GetHeight(),0)
				grps.RotateTransform(90);

				xPos = rcItem.top;
				yPos = -rcItem.left;
			}
			else if ( event.tGestureInfo.dblAngle == 180 )
			{
				grps.ResetTransform();   //恢复默认绘制
				grps.TranslateTransform( pImg->GetWidth()*event.tGestureInfo.dblZoomRatio, pImg->GetHeight()*event.tGestureInfo.dblZoomRatio ); // 将原点移动到(pImg->GetWidth(), pImg->GetHeight())
				grps.RotateTransform(180);

				xPos = -rcItem.left;
				yPos = -rcItem.top;
			}
			else if ( event.tGestureInfo.dblAngle == 270 )
			{
				grps.ResetTransform();   //恢复默认绘制
				grps.TranslateTransform( 0, pImg->GetWidth()*event.tGestureInfo.dblZoomRatio ); // 将原点移动到(0,pImg->GetWidth())
				grps.RotateTransform(270);
		...
		grps.DrawImage( pImg, xPos + ptInCtl.x, yPos + ptInCtl.y, nImgWidth*event.tGestureInfo.dblZoomRatio, nImgHeight*event.tGestureInfo.dblZoomRatio );

		event.tGestureInfo.emGestureType = emGestureDefaut;
	}
