...
CLabelUI::CLabelUI() : m_uTextStyle(DT_VCENTER), m_dwTextColor(0), 
    m_dwDisabledTextColor(0), m_iFont(-1), m_bShowHtml(false), m_bTiledX(false), m_bTiledY(false),
	m_bIsLabelHorMosaic(false), m_nLabelMosaicPos(0)
{
    ::ZeroMemory(&m_rcTextPadding, sizeof(m_rcTextPadding));
	memset( m_chImage, 0, MAX_PATH );
}
...
{
    m_iFont = index;
 	//可保持原样，暂不绘制2013.5.3 yjj  Invalidate();//改变字体后，需要重新绘制，否则方案无效 2013.5.3 yjj 
}
...
void CLabelUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
 ...
 //   }
	...
	else if( _tcscmp(pstrName, _T("isstretch")) == 0 )		//2013 06 06 by ldy  label中图片拉伸控制
	{
		if( _tcscmp( pstrValue, _T("true") ) == 0 )
		{
			m_bTiledX = m_bTiledY = true;
		}
	}
	else if( _tcscmp(pstrName, _T("labelimagebk")) == 0 )   //2013 06 03 by ldy  label中加入背景图绘制
	{
		wcscat( m_chImage, pstrValue );
    }
	else if( _tcscmp(pstrName, _T("islabelhormosaic")) == 0 )   //label是否拼接，若不拼接，则只画一次  by xhx
	{
		if( _tcscmp( pstrValue, _T("true") ) == 0 )
		{
			SetLabelHorMosaic(true);
		}
	}
	else if( _tcscmp(pstrName, _T("labelmosaicpos")) == 0 )
	{
		m_nLabelMosaicPos = _ttoi(pstrValue);
	}
...
void CLabelUI::SetImage(LPCTSTR pStrImage, bool bStretch )
{
	memset( m_chImage, 0, MAX_PATH );
	wcscat( m_chImage, pStrImage );

	m_bTiledX = m_bTiledY = bStretch;	
...
{
	if( wcslen(m_chImage) == 0 ) return;

	//DrawImage( hDC, (LPCTSTR)m_chImage, NULL, m_bTiledX, m_bTiledY );
	if ( GetLabelHorMosaic() )
	{
		int nFixedWidth = m_rcItem.right - m_rcItem.left - m_nLabelMosaicPos;
		CControlUI::MosaicHoriImage(hDC, (LPCTSTR)m_chImage, (LPCTSTR)m_chImage, (LPCTSTR)m_chImage,
			nFixedWidth,  TRUE, CPoint( m_nLabelMosaicPos, 0 ) );
	}
	else
	{
		TEventUI event;
		CControlUI::DrawImageGDIPlus( hDC,  event, CPoint(0, 0), false, (LPCTSTR)m_chImage );
	}
}
...
//
//
CListIconUI::CListIconUI() : m_nIndex(-1), m_bSelState(false), m_strItemName(_T(""))
{
	memset( m_chSelImage, 0, MAX_PATH );
	m_pManageUI = NULL;
}

LPCTSTR CListIconUI::GetClass() const
{
	return _T("ListIconUI");
}

LPVOID CListIconUI::GetInterface(LPCTSTR pstrName)
{
	if( _tcscmp(pstrName, _T("ListIconUI")) == 0 ) return static_cast<CListIconUI*>(this);
	return CLabelUI::GetInterface(pstrName);
}

bool CListIconUI::Activate()
{
	...
	if( NULL != m_pManager )
	{
		CCommonListUI* pList = static_cast<CCommonListUI*>( m_pManageUI->FindControl(m_strItemName.c_str()) );
		if ( NULL != pList )
		{
			pList->SetCheckStateMap( m_nIndex, m_bSelState );
			m_pManager->SendNotify( /*this*/pList->GetItemAt( m_nIndex ), DUI_MSGTYPE_ITEMCLICKICON, (WPARAM)m_nIndex, (LPARAM)m_bSelState );
		}
	}
...
void CListIconUI::DoEvent(TEventUI& event)
{
	if( event.Type == UIEVENT_SETFOCUS ) 
	{
		//Invalidate();
	...
	{
		SetLabelSelState( !m_bSelState );
		Activate();
		//Invalidate();  //即时刷新
		return;
	}
	if ( event.Type != 9 && event.Type != 10 && event.Type != 11 )
	{
		int i = 0;
	}
...
void CListIconUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	if( _tcscmp(pstrName, _T("labelselimagebk")) == 0 )   //label中选中状态的背景图绘制
	{
		wcscat( m_chSelImage, pstrValue );
	}

	else CLabelUI::SetAttribute(pstrName, pstrValue);
}

void CListIconUI::PaintBkImage( HDC hDC )
{
	//DrawImage( hDC, (LPCTSTR)m_chImage, NULL, m_bTiledX, m_bTiledY );
	if ( GetLabelHorMosaic() )
	{
	...
	{
		const TImageInfo* dataImage = NULL;
		TEventUI event;
		if ( !m_bSelState )
		{
			if( wcslen(m_chImage) == 0 ) return;
			dataImage = const_cast<TImageInfo*>(m_pManager->GetImageEx(m_chImage));
			CControlUI::DrawImageGDIPlus( hDC,event, CPoint(0, 0),false, (LPCTSTR)m_chImage );
		} 
		else
		{
			if( wcslen(m_chSelImage) == 0 ) return;
			dataImage = const_cast<TImageInfo*>(m_pManager->GetImageEx(m_chSelImage));
			CControlUI::DrawImageGDIPlus( hDC,event, CPoint(0, 0),false, (LPCTSTR)m_chSelImage );
		}

		if( NULL == dataImage ) return; 

		//设置点击CListIconUI的响应区域和刷新区域
		m_rcItem.right = m_rcItem.left + dataImage->nX;
		m_rcItem.bottom = m_rcItem.top + dataImage->nY;
	}
}

void CListIconUI::SetLabelSelState(bool bSelState)
{ 
	m_bSelState = bSelState; 
	Invalidate();
}
...
{
    if( _tcscmp(pstrName, _T("fgimage")) == 0 ) SetForeImage(pstrValue);
...
m_uButton2State(0), m_uThumbState(0), m_bShowButton1(true), m_bShowButton2(true)/*, m_bIsInitialized(false)*/
{
...
LPCTSTR CScrollBarUI::GetMosaicSliderImage()
{
	return m_strMosaicSliderImg.c_str();
}
...
void CScrollBarUI::DoPaint(HDC hDC, const RECT& rcPaint)
{
	...
	if ( this->m_pParent->GetMosaicBk() )
	{
		PaintScrollBk(hDC);
		PaintScrollButton1(hDC);
		PaintScrollButton2(hDC);
...
{
	m_strBkgBottom = m_strMosaicSliderImg.c_str();
	m_strBkgBottom += _T("\\");
	m_strBkgBottom += pBkgBottom;

	m_strBkgMiddle = m_strMosaicSliderImg.c_str();
	m_strBkgMiddle += _T("\\");
	m_strBkgMiddle += pBkgMiddle;

	m_strBkgTop = m_strMosaicSliderImg.c_str();
	m_strBkgTop += _T("\\");
	m_strBkgTop +=	pBkgTop;

	m_strDownBottom = m_strMosaicSliderImg.c_str();
	m_strDownBottom += _T("\\");
	m_strDownBottom += pDownBottom;

	m_strDownMiddle = m_strMosaicSliderImg.c_str();
	m_strDownMiddle += _T("\\");
	m_strDownMiddle += pDownMiddle;

	m_strDownTop = m_strMosaicSliderImg.c_str();
	m_strDownTop += _T("\\"); 
	m_strDownTop += pDownTop;

	m_strNormalBottom = m_strMosaicSliderImg.c_str();
	m_strNormalBottom += _T("\\");
	m_strNormalBottom += pNormalBottom;

	m_strNormalMiddle = m_strMosaicSliderImg.c_str();
	m_strNormalMiddle += _T("\\");
	m_strNormalMiddle += pNormalMiddle;

	m_strNormalTop = m_strMosaicSliderImg.c_str();
	m_strNormalTop += _T("\\");
	m_strNormalTop += pNormalTop;

	m_strNormalBtn1 = m_strMosaicSliderImg.c_str();
	m_strNormalBtn1 += _T("\\");
	m_strNormalBtn1 += pNormalBtn1;

	m_strDownBtn1 = m_strMosaicSliderImg.c_str();
	m_strDownBtn1 += _T("\\");
	m_strDownBtn1 += pDownBtn1;

	m_strNormalBtn2 = m_strMosaicSliderImg.c_str();
	m_strNormalBtn2 += _T("\\");
	m_strNormalBtn2 += pNormalBtn2;

	m_strDownBtn2 = m_strMosaicSliderImg.c_str();
	m_strDownBtn2 += _T("\\");
	m_strDownBtn2 += pDownBtn2;

	TImageInfo* pImgBkg = const_cast<TImageInfo*>(m_pManager->GetImageEx(m_strBkgMiddle.c_str()) );
	if ( NULL != pImgBkg )
	{
	...
	TImageInfo* pImgNormalBtn1 = const_cast<TImageInfo*>(m_pManager->GetImageEx(m_strNormalBtn1.c_str()) );
	if ( NULL == pImgNormalBtn1 )
	{
		SetShowButton1(false);
	}

	TImageInfo* pImgNormalBtn2 = const_cast<TImageInfo*>(m_pManager->GetImageEx(m_strNormalBtn2.c_str()) );
...
void CScrollBarUI::PaintScrollBk(HDC hDC)
{
	...
	if( !m_strBkgTop.empty() && !m_strBkgBottom.empty() && !m_strBkgMiddle.empty() ) 
	{
		CControlUI::MosaicVertImage(hDC, m_strBkgTop.c_str(), m_strBkgBottom.c_str(), m_strBkgMiddle.c_str(), nFixedHeight);
		return;
	}
}

void CScrollBarUI::PaintScrollButton1(HDC hDC)
{
	...
	TImageInfo* pBtnImg = const_cast<TImageInfo*>(m_pManager->GetImageEx(m_strNormalBtn1.c_str()) );
	if ( NULL == pBtnImg ){ return; }

	TImageInfo* pBkImg = const_cast<TImageInfo*>(m_pManager->GetImageEx(m_strBkgMiddle.c_str()) );
	if ( NULL == pBkImg ){ return; }

	int nWidthDiff = (pBkImg->nX - pBtnImg->nX) / 2;
	TEventUI event;
	if( (m_uButton1State & UISTATE_PUSHED) != 0 ) 
	{
		if( !m_strDownBtn1.empty() )
		{
			CControlUI::DrawImageGDIPlus( hDC, event, CPoint( m_rcButton1.left - m_rcItem.left + nWidthDiff, m_rcButton1.top - m_rcItem.top + 3 ), false, (LPCTSTR)m_strDownBtn1.c_str() );
			return;
		}
	}

	if( !m_strNormalBtn1.empty() ) 
	{
		CControlUI::DrawImageGDIPlus( hDC, event, CPoint( m_rcButton1.left - m_rcItem.left + nWidthDiff, m_rcButton1.top - m_rcItem.top + 3 ), false, (LPCTSTR)m_strNormalBtn1.c_str() );
		return;
	}
}

void CScrollBarUI::PaintScrollButton2(HDC hDC)
{
	...
	TImageInfo* pBtnImg = const_cast<TImageInfo*>(m_pManager->GetImageEx(m_strNormalBtn2.c_str()) );
	if ( NULL == pBtnImg ){ return; }
	...
	{
		if( !m_strDownBtn2.empty() )
		{
			CControlUI::DrawImageGDIPlus( hDC, event, CPoint( m_rcButton2.left - m_rcItem.left + nWidthDiff, m_rcButton2.bottom - m_rcItem.top - pBtnImg->nY - 3 ), false, (LPCTSTR)m_strDownBtn2.c_str() );
			return;
		}
	}

	if( !m_strNormalBtn2.empty() ) 
	{
		CControlUI::DrawImageGDIPlus( hDC, event, CPoint( m_rcButton2.left - m_rcItem.left + nWidthDiff, m_rcButton2.bottom - m_rcItem.top - pBtnImg->nY - 3 ), false, (LPCTSTR)m_strNormalBtn2.c_str() );	
		return;
	}
}
...
void CScrollBarUI::PaintScrollThumb(HDC hDC)
{
	...
	TImageInfo* pThumbImg = const_cast<TImageInfo*>(m_pManager->GetImageEx(m_strNormalMiddle.c_str()) );
	if ( NULL == pThumbImg ){ return; }

	TImageInfo* pBkImg = const_cast<TImageInfo*>(m_pManager->GetImageEx(m_strBkgMiddle.c_str()) );
	if ( NULL == pBkImg ){ return; }

	int nWidthDiff = (pBkImg->nX - pThumbImg->nX) / 2;
	int nHeight = m_rcThumb.bottom - m_rcThumb.top;

	if( (m_uThumbState & UISTATE_PUSHED) != 0 ) 
	{
		if( !m_strDownTop.empty() && !m_strDownBottom.empty() && !m_strDownMiddle.empty() ) 
		{			
			CControlUI::MosaicVertImage(hDC, m_strDownTop.c_str(), m_strDownBottom.c_str(), m_strDownMiddle.c_str(), 
				nHeight, TRUE, CPoint(m_rcThumb.left - m_rcItem.left + nWidthDiff, m_rcThumb.top - m_rcItem.top));
			return;
		}
	}

	if( !m_strNormalTop.empty() && !m_strNormalBottom.empty() && !m_strNormalMiddle.empty() ) 
	{
		CControlUI::MosaicVertImage(hDC, m_strNormalTop.c_str(), m_strNormalBottom.c_str(), m_strNormalMiddle.c_str(), 
			nHeight, TRUE, CPoint(m_rcThumb.left - m_rcItem.left + nWidthDiff, m_rcThumb.top - m_rcItem.top));
		return;
	}
}
