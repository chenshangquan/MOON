...
bool CCaseManager::AddCase(   CMarkupNode* pNodeRoot, CPaintManagerUI* pManager )
{
	...
	CStdString strCaseName = pNodeRoot->GetAttributeValue( _T("name") );
	 
	CtlsAttribMap ctrlMap;
	for(  CMarkupNode node = pNodeRoot->GetChild(); node.IsValid(); node = node.GetSibling() ) 
	{//控件集
		/*CStdString*/tString pstrCtrlName = node.GetName();//
		VcAttrib vcAttrib;
...
bool CCaseManager::DoCase( const LPCTSTR szCaseName, CPaintManagerUI* pManager )
{
	bool Re = false;
	...
	{//根据方案名找见方案
		...
		{   
			...
			CControlUI* pCtrl = pManager->FindControl(itCtlsAttrib->first.c_str()  );
			if( NULL == pCtrl )
			{
				itCtlsAttrib ++;
			...
			{  
				pCtrl->SetAttribute( itVcAttrib->strPropName.c_str(), itVcAttrib->strPropValue.c_str()); 
				itVcAttrib++;
			} 	
