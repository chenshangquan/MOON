...
void CRichEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("drawimage")) == 0 )
	{
		m_strDrawImage = pstrValue;
	}
...
{
	if( m_strDrawImage.size() != 0 ) 
	{
		if( !DrawImage(hDC, (LPCTSTR)m_strDrawImage.c_str(), NULL, true, true ) );// m_strDrawImage.empty();
	}
}
