...
CRkcConfigCtrl::CRkcConfigCtrl(CRkcSession &cSession) : CRkcConfigCtrlIF()
{
    ...
    memset(&m_tRK100BrdInfo, 0, sizeof(TRK100BrdInfo));
...
void CRkcConfigCtrl::BuildEventsMap()
{
    ...
    REG_PFUN( RK100_EVT_GET_BRD_INFO_ACK, CRkcConfigCtrl::OnGetBrdInfoConfigRsp );
...
u16 CRkcConfigCtrl::GetBrdInfoConfig()
{
    TRK100MsgHead tRK100MsgHead;//定义消息头结构体
    memset(&tRK100MsgHead,0,sizeof(TRK100MsgHead));
    //整型传数据集的转网络序
    tRK100MsgHead.dwEvent = htonl(RK100_EVT_GET_BRD_INFO);
    CRkMessage rkmsg;//定义消息
    rkmsg.SetBody(&tRK100MsgHead, sizeof(TRK100MsgHead));//添加头内容

    PrtRkcMsg( RK100_EVT_GET_BRD_INFO, emEventTypeScoketSend ,"获取板卡配置");

    SOCKETWORK->SendDataPack(rkmsg);//消息发送
    return NOERROR;
}

void CRkcConfigCtrl::OnGetBrdInfoConfigRsp(const CMessage& cMsg)
{
    TRK100MsgHead tMsgHead = *reinterpret_cast<TRK100MsgHead*>( cMsg.content );
    tMsgHead.dwEvent = ntohl(tMsgHead.dwEvent);
    tMsgHead.dwHandle = ntohl(tMsgHead.dwHandle);
    ...
    PrtRkcMsg( RK100_EVT_GET_BRD_INFO_ACK, emEventTypeScoketRecv, "wOptRtn = %d", tMsgHead.wOptRtn);

    if (tMsgHead.wMsgLen != 0)
    {
        m_tRK100BrdInfo = *reinterpret_cast<TRK100BrdInfo*>( cMsg.content + sizeof(TRK100MsgHead) );
        m_tRK100BrdInfo.wHid = ntohs(m_tRK100BrdInfo.wHid);
        m_tRK100BrdInfo.wPid = ntohs(m_tRK100BrdInfo.wPid);
        m_tRK100BrdInfo.dwExt = ntohl(m_tRK100BrdInfo.dwExt);
    }
    PostEvent(UI_RKC_BRDINFO_NTY, WPARAM(RK100_OPT_RTN_OK == tMsgHead.wOptRtn), (LPARAM)tMsgHead.wOptRtn );
}

u16 CRkcConfigCtrl::GetBrdInfoConfig(TRK100BrdInfo& tRK100BrdInfo)
{
    tRK100BrdInfo = m_tRK100BrdInfo;
    return NOERROR;
}
