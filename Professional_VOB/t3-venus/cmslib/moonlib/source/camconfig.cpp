...
//////////////////////////////////////////////////////////////////////
CCamConfig::CCamConfig(CMoonSession &cSession) : CCamConfigIF()
{
	m_pSession = &cSession;
...
{   
	...
	REG_PFUN( ev_TpMoonCamInfo_Nty, OnMoonCamCfgNty );
	REG_PFUN( ev_TpMechanismSelect_Nty, OnMechanismSelectNty );
	REG_PFUN( ev_TpMechVer_Nty, OnTpMechverNty );
	REG_PFUN( ev_TpCamSelect_Nty, OnCamSelNty );
	REG_PFUN( ev_CamParaSynchronize_Ind, OnCamParaSynchronizeInd );
	REG_PFUN( ev_TpCamSelect_Ind, OnCamSelInd );
	REG_PFUN( ev_TpMechanismSelect_Ind, OnCamStyleSelInd );
	REG_PFUN( ev_TpCamSetGamma_Ind, OnGammaSelInd );
	REG_PFUN( ev_TpCamSetZoonStart_Ind, OnCamZoomInd );
	REG_PFUN( ev_TpCamSetZoonStop_Ind, OnCamZoomStopInd );
	REG_PFUN( ev_TpCamSetDrectZoon_Ind, OnCamZoomValueInd );
	REG_PFUN( ev_TpCamFocusMode_Ind, OnCamAutoFocusInd );
	REG_PFUN( ev_TpCamFocusNear_Ind, OnCamFocusNearInd );
	REG_PFUN( ev_TpCamFocusStop_Ind, OnCamFocusStopInd );
	REG_PFUN( ev_TpCamFocusFar_Ind, OnCamFocusFarInd );
	REG_PFUN( ev_TpCamExpMode_Ind, OnAutoExposureInd );
	REG_PFUN( ev_TpCamExpAperture_Ind, OnCamApertureInd );
	REG_PFUN( ev_TpCamExpGain_Ind, OnCamGainInd );	
	REG_PFUN( ev_TpCamExpSOrThShutter_Ind, OnCamShutSpdCInd );
	REG_PFUN( ev_TpCamExpFOrTwShutter_Ind, OnCamTwShutSpdCInd );
	REG_PFUN( ev_TpCamWhiteBalanceMode_Ind, OnCamAutoWBInd );
	REG_PFUN( ev_TpCamWhiteBalanceRGain_Ind, OnCamRGainInd );
	REG_PFUN( ev_TpCamWhiteBalanceBGain_Ind, OnCamBGainInd );
	REG_PFUN( ev_TpCamImagePara_Ind, OnCamImageParaInd ); 
	REG_PFUN( ev_TpCamBackLight_Ind, OnCamLightPensationInd );
	REG_PFUN( ev_TpCamReduNoise_Ind, OnCamNRInd );
	REG_PFUN( ev_TpSetCam2DNoiseLevle_Ind, OnCam2DNRInd );
	REG_PFUN( ev_TpSetCam3DNoiseLevle_Ind, OnCam3DNRInd );
	REG_PFUN( ev_TpCamInitPreset_Ind, OnCamOrderPosSelInd );
	REG_PFUN( ev_TpCamPreSet_Nty, OnCamPreSetNty );
	REG_PFUN( ev_TpCamPreSet_Ind, OnCamPreSetInd );
	REG_PFUN( ev_TpCamPreReset_Ind, OnCamPreResetInd );
	REG_PFUN( ev_TpCamPreRecall_Ind, OnCamPreRecallInd );
	REG_PFUN( ev_TpCamPreModify_Ind, OnCamPreSetModifyInd );	
	REG_PFUN( ev_TpCamParamDefault_Ind, OnCamParamDefaultInd );
	REG_PFUN( ev_TpImageDistortCorrect_Ind, OnCamDistortAdjustInd );
	REG_PFUN( ev_TPChangeSideRGB_Nty, OnChangeSideRGBNty );
	REG_PFUN( ev_TPDirectSetColorRGB_Ind, OnSetColorRGBDirectInd );
	REG_PFUN( ev_TPSetColorRGB_Ind, OnSetColorRGBInd );
	REG_PFUN( ev_TPChooseRectangle_Ind, OnChooseRectangleInd );
	REG_PFUN( ev_TPAutoSetColorConstancy_Ind, OnSetAutoColorConstancyInd );
	REG_PFUN( ev_TpMoonUpgrade_Ind, OnMoonUpgradeInd );	
	REG_PFUN( ev_CamUpgrade_Ind, OnUpdateMechanismInd );
	REG_PFUN( ev_TpSetRefer_Ind, OnReferenceLineInd );
	REG_PFUN( ev_TpSetRefer_Nty, OnReferenceLineNty );
	REG_PFUN( ev_TpFlatTurn_Ind, OnHorizontalFlipInd );
	REG_PFUN( ev_TpFlatTurn_Nty, OnHorizontalFlipNty );
	REG_PFUN( ev_TpDisortSwitch_Ind, OnTrapezoidalCorInd );
	REG_PFUN( ev_TPMoonShowRectRegion_Ind, OnShowRectRegionInd );
	REG_PFUN( ev_TpColorSwich_Ind, OnColorSwitchInd );
	REG_PFUN( ev_TpSetDisort_Ind, OnTraCorParamInd );
	REG_PFUN( ev_TpCamSnapshot_Ind, SnapShotInd );
	REG_PFUN( ev_TPChangeDistor_Ind, OnChangeDistortInd );
	REG_PFUN( ev_TPDirectSetDistor_Ind, OnDirectSetDistortInd );
	REG_PFUN( ev_TpSecDefault_Nty, OnSecDefaultNty );
	REG_PFUN( ev_TpSecBoundary_Nty, OnSecBoundaryNty );
	//延时矫正
    REG_PFUN( ev_TpFpgaDelayCorrect_Ind, OnDelayCorrectInd );
	REG_PFUN( ev_TpFpgaDelayCorrect_Nty, OnDelayCorrectNty );
}
...
{
	PrtMsg( OSP_DISCONNECT, emEventTypemoontoolRecv,"[CCamConfig::OnLinkBreak]清空配置信息" );	
...
// later added
//	PostEvent( UI_MOONTOOL_SET_CAMERA_ZOOM_IND, /*(WPARAM)emZoom*/0, (LPARAM)bCamZoom );
}
...
void CCamConfig::OnCamPreRecallInd( const CMessage& cMsg )
{
	...
	PrtMsg( ev_TpCamPreRecall_Ind, emEventTypemoontoolRecv, "Index:%d, bOk:%d, achachDescription:%s", tTPCamPre.byPre, bOk, tTPCamPre.achachDescription  );	
}
