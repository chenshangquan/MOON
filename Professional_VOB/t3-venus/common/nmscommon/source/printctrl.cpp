...
void CPrintCtrl::ForcePrtMsg( u32 dwMsgID, EmEventType emEventType,char * format,... )
{   
    //打印消息名 
...
void  CPrintCtrl::PrintMsg(u32 dwMsgID,EmEventType emEventType, char* format,...)
{  
...
BOOL32 CPrintCtrl::IsPrintMsg( const u32 dwMsgID )
{ 
	if ( m_bPrintAllMsg )
	{
		return TRUE;
...
CString CPrintCtrl::GetEventTypeDescribe(EmEventType emType )
{
    ...
    case  emEventTypecnstoolRecv:
        str = "[收到 CNS 消息]";
	    break;
	case  emEventTypemoontoolSend:
		str = "[发送 MOON 消息]";
		break;
	case  emEventTypemoontoolRecv:
		str = "[收到 MOON 消息]";
		break;
    default:
        ;
