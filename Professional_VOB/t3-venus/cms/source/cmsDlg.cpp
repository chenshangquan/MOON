...
BEGIN_MESSAGE_MAP(CCmsDlg, CDialog)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_TEST_CNS, OnBtnTestCns) 
	ON_BN_CLICKED(IDC_BTN_TEST_UMS, OnBtnTestUms)
	ON_BN_CLICKED(IDC_BTN_TEST_UIFACTORY, OnBtnTestUifactory)
...
void CCmsDlg::OnBtnTestCns()
{
 
}
 

void CCmsDlg::OnBtnTestUms() 
{ 
	
}

void CCmsDlg::OnBtnTestUifactory() 
{
	//IWndTree* pTree  = const_cast<IWndTree*>(CWindowManage::GetSingletonPtr()->GetWindowTree( "system.xml" ));
	IWndTree* pTree  = const_cast<IWndTree*>(UIManagePtr->GetWindow( "TPDlg"));
	int o = 0;
}
...
//窗口启动时自动隐藏
void CCmsDlg::OnWindowPosChanging(WINDOWPOS   FAR*   lpwndpos) 
{ 
